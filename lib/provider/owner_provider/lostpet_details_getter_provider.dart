...
class LostPetDetailsGetterOwner extends ChangeNotifier {
  ...
  Future<void> getOwnerDetails() async {
    ownerData =
        await _firestoreServiceOwner.getOwnerDetailsByEmail(_currentUserEmail!);

    print("Owner Details: $ownerData");

    print("\n owner data address: ${ownerData['address'][0]['coordinates']}");
  }
  ...
  Future<void> calculateDistances() async {
    if (ownerData == null) {
      print('Owner data is not available.');
      return;
    }

    String ownerCoordinates = ownerData['address'][0]['coordinates'];
    print("Owner Coordinates: $ownerCoordinates");

    for (var pet in _allLostPets) {
      String petCoordinates = pet['address']['coordinates'];
      String distance =
          await _calculateDistance(ownerCoordinates, petCoordinates);
